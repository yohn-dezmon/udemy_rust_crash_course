...
enum Shot {
    Bullseye,
    Hit(f64),
    Miss, // what are these exactly, types?
    // what are variants, are they types?
}
...
fn main() {
    ...
    for arrow_coord in arrow_coords {
        let desc = arrow_coord.print_description();

        let distance_from_center = arrow_coord.distance_from_center();
