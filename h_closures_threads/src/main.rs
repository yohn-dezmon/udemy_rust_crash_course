...
fn expensive_sum(v: Vec<i32>) -> i32 {
    ...
    v.iter()
        .filter(|x| { *x % 2 == 0 }) // why do I need to dereference again? bc the closure will be passed references to the arguments 
        // by dereferencing you are getting access to get the values themselves
        .map(|&x| x * x) // &x dereferences the argument within the parameter list
        .sum()
}
...
fn main() {
    ...
    let handle = thread::spawn(move || {
        // expensive_sum takes `my_vector` by value, so the closure will automaticaly be a `move` closure 
    ...
    // tx is a `Sender` object. Used for sending messsages between threads.
    // rx is a `Receiver` object, can receive those messages on a separate thread.
    ...
    // the child threads.
    let (tx, rx) = channel::unbounded();
    // necessary if you want to send an end of a channel across threads...
    let rx1 = rx.clone();
    let rx2 = rx.clone();

    let child_thread_1 = thread::spawn(move || {
        for msg in rx1 {
            println!("child_thread_1: Received {}", msg)
        }
    });

    let child_thread_2 = thread::spawn(move || {
        for msg in rx2 {
            println!("child_thread_2: Received {}", msg)
        }
    });
    ...
    // close the channel to indicate no more messages will be sent
    drop(tx);
