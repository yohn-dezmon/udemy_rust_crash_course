...
fn inspect(ex_str: &String) {
    if ex_str.ends_with("s") {
        println!("{} is plural!", ex_str);
    } else {
        println!("{} is singular!", ex_str);
    }
}

fn change(ex_str: &mut String) {
    if !ex_str.ends_with("s") {
        ex_str.push_str("s");
    }
}

fn eat(ex_str: String) -> bool {
    if ex_str.starts_with("b") && ex_str.contains("a") {
        true
    } else {
        false
    }
...
fn bedazzle(ex_str: &mut String) {
    // * to dereference in order to manupliate the value
    *ex_str = String::from("sparkly");
}